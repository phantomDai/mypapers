...
Adaptive random testing (ART)~\cite{chen2010adaptive, chen2009adaptive, chen2013code, chen2017similarity, chen2018test, 8846002}, for example, has been proposed to improve RT by attempting to have a more diverse distribution of test cases in the input domain.
...
Because it is likely that they can be complementary to each other, detecting different faults, it is intuitively appealing to investigate their integration \textcolor{red}{in random partition testing (RPT)}.
...
In this paper, we present a \textcolor{red}{DRT} approach for web services, as an enhanced version of RT adapted to the context of SOA.
...
\begin{itemize}
  ...
  Normally, web service owners will not make \textcolor{red}{the} source code of their web services accessible.
  ...
  Unlike traditional software testing where testers can control the execution of \textcolor{red}{the} software under test, there is usually no opportunity to intervene in the execution of the web service under test, which is often deployed in a remote service container.
...
In this section, we describe a framework for applying DRT to web services, discuss guidelines for \textcolor{red}{setting} DRT's parameters, and present a prototype that partially automates  DRT for web services.
...
\begin{enumerate}[1)]
  ...
  Testers need to initialize the test profile, a simple way of doing which would be the use of a uniform probability distribution ($p_1 = p_2 = \ldots = p_k = 1 / k$, where $k$ denotes the number of partitions, and $p_i$ $(i = 1,2, \ldots, k)$ denotes the probability of selecting the $i^{th}$ partition). They also need to set \textcolor{red}{the} DRT parameters (guidelines for which are introduced in Section \ref{sec:relation}).
  ...
  Upon completion of each test, its pass or fail status is determined by comparing the actual and expected results (\textcolor{red}{a pass status} if both are the same).
...
\begin{enumerate}[1)]
  ...
  \item
  Test cases are selected with replacement, which means that \textcolor{red}{some} test cases may be selected more than once.
\end{enumerate}
...
To explore the relationship between $p_i^{n + 1}$ and $p_i^{n}$, we calculate the conditional probability, $p(i|\delta)$, of the following four situations (denoted \textcolor{red}{as} $\delta_1, \delta_2, \delta_3$, and $\delta_4$):
...
When $\theta_{min} < \theta_M < \frac{1}{2}$, the value of $\mathcal{I}_{upper}$ should \textcolor{red}{be} close to the lower bound of $\mathcal{I}$.
...
\begin{enumerate}[1)]
  ...
  \item
  \emph{Configuration}. This feature, \textcolor{red}{as shown in Figure 2,} interacts with the testers to obtain and set the information related to testing the web service, including: the address of the web service under test; the DRT parameters and partition details; and the test case preparation. The detailed steps are as follows:
...
\begin{figure}
  ...
  \caption{\textcolor{red}{DRTester configuration snapshots}}
...
\textcolor{red}{Although a number of web services are publicly available, for various reasons, their implementations are not. This renders them unsuitable for our experiments, which involve the creation of faulty mutants (requiring access to the implementations). We therefore} selected three web services as the subject programs for our study, and implemented them ourselves, based on real-life specification\footnote{The implementations have been made available at: https://github.com/phantomDai/subjects4tsc.git}:
\texttt{Aviation Consignment Management Service (ACMS)};
\texttt{China Unicom billing service (CUBS)}; and
\texttt{Parking billing service (PBS)}. We used the tool MuJava \cite{ma2005mujava} to conduct mutation analysis \cite{demillo1978hints, sun2017path, Chang2019A}, generating a total of 1563 mutants. Each mutant was created by applying a syntactic change (using one of all applicable mutation operators provided by MuJava) to the original program. Equivalent mutants, and those that were too easily detected
...
On the other hand, testers may also use past experience to \textcolor{red}{help guide selection} of a different probability distribution as the initial profile. In our experiment, we used a uniform probability distribution for the initial test profile. The initial test
...
Furthermore, as will be discussed in Section~\ref{sec:results}, \textcolor{red}{we also conducted statistical tests to confirm the significance of the results.}
...
Table~\ref{tableHlom:f/f2/t-measure} clearly shows that the \textcolor{red}{differences} between \textcolor{red}{pairs} of testing techniques \textcolor{red}{are all significant}.
...
Table \ref{tableHlom:f/f2/t-time} shows that\textcolor{red}{:} a) DRT was significantly better than AT in terms of F-/F2-/T-time; b) DRT was significantly better than RT and RPT in terms of F2-/T-time; and c) DRT marginally outperformed RT and RPT in terms of F-time.
...
\begin{itemize}
  \item
  While DRT outperformed RT and RPT in terms of fault detection effectiveness and efficiency, this \textcolor{red}{was} achieved at the cost of the additional effort required to set the partitions and test profiles.
...
All \textcolor{red}{of} the above approaches aim to generate test cases without considering the impact of test case execution order on test efficiency.
...
Our approach \textcolor{red}{involves} software cybernetics and PT:
In DRT, selection of a partition is done according to the testing profile, which is updated throughout the \textcolor{red}{testing} process.
...
Most web service testing techniques assume that the computed output for any test case is verifiable, which is\textcolor{red}{,} however\textcolor{red}{,} not always true in practice (\textcolor{red}{a situation} known as the oracle problem \cite{barr2015oracle, patel2018mapping}). Thus, many testing techniques may not be applicable in some cases.
\textcolor{red}{To address the oracle problem for testing web services, Sun et al. \cite{sun2011} proposed a metamorphic testing \cite{chen1998metamorphic, chen2018metamorphic} approach that not only alleviates the oracle problem, but is also a practical and efficient option for testing web services. They conducted a case study that showed that up to $94.1\%$ of seeded faults could be detected without the need for oracles.}

\subsection{Improving RT and PT}
\label{sec:relatedworkRT}

Based on the observation that failure-causing inputs tend to cluster into contiguous regions in the input domain~\cite{Ammann88, Finelli91}, \textcolor{red}{much work has} been done to improve RT \cite{cai2009random, chen2010adaptive, 8846002}.
...
We proposed a framework that examines key issues when applying DRT to test web services, and developed a prototype to make the method \textcolor{red}{practical} and effective.
...
In our future work, we plan to conduct experiments on more web services to further validate \textcolor{red}{the} effectiveness, and identify the limitations of our method.
