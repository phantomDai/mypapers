...
\begin{enumerate}[1)]
  ...
  \item
  \emph{Configuration}. This feature interacts \textcolor{red}{(as shown in Figure \ref{fig:configuration})} with the te{}sters to obtain and set the information related to testing the web service, including: the address of the web servic{}e under test; the DRT parameters and partition details; and the test case preparation. The detailed steps are as follows:
...
all of them can be found at the repository: https://github.com/phantomDai/subjects4tsc.git. 
...
\textcolor{red}{Besides}, 18 distinct faults were used to evaluate the performance.These faults cover different types of mutation operators and require an average of more than 20 randomly generated test cases to be detected.
\textcolor{red}{Furthermore, the performance of DRT can be affected by some factors such as partition scheme and probability adjusting factor. It is difficult and unrealistic to demand that DRT always performances better than RT or PT in any scenario for every web service. We, therefore, want to verify the performance of DRT in different scenarios. Accordingly, we created 2 partition schemes and set 13 kinds of adjusting factors to improve the generalisability for the findings.
Note that the tool we developed cannot affect the performance of DRT, which just makes it easier for testers to use our method and improves test efficiency.}
\sout{Although we have tried to improve the generalisability of the findings by applying different partitioning granularities, and 13 kinds of parameters, we cannot be certain that similar results would be observed in other types of web services.}
