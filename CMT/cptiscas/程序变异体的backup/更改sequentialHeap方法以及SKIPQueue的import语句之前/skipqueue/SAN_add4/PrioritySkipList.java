 ...
 */
package mutants.SkipQueue.SAN_add4;
...
public final class PrioritySkipList<T> implements Iterable<T> {
  ...
  private static int randomLevel() {
    ...
    x ^= x >>> 17;
    randomSeed = x ^= x << 5;
    if ((x & 0x80000001) != 0) // service highest and lowest bits
      return 0;
    int level = 1;
    while (((x >>>= 1) & 1) != 0) ++level;
        ...
        for (int level = bottomLevel+1; level <= node.topLevel; level++) {
          while (true) {
            ...
            if (pred.next[level].compareAndSet(succ, node, false, false))
              break;
            addfind(node, preds, succs); // find new preds and succs
          }
        }
        return true;
      }
  ...
  boolean addfind(Node<T> node, Node<T>[] preds, Node<T>[] succs) {
    int bottomLevel = 0;
    boolean[] marked = {false};
    boolean snip;
    Node<T> pred = null, curr = null, succ = null;
    ...
    while (true) {
      ...
      for (int level = MAX_LEVEL; level >= bottomLevel; level--) {
        ...
        while (true) {
          succ = curr.next[level].get(marked);
          while (marked[0]) {           // replace curr if marked
            if (pred.next[level].getReference() == curr && pred.next[level].isMarked() == false ){
  ...
  boolean removefind(Node<T> node, Node<T>[] preds, Node<T>[] succs) {
    int bottomLevel = 0;
    boolean[] marked = {false};
    boolean snip;
    Node<T> pred = null, curr = null, succ = null;
