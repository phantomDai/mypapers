...
package mutants.SequentialHeap.RSK_add1;
...
public class SequentialHeap<T> implements PQueue<T> {
  ...
  public synchronized T removeMin() {
    ...
    while (parent < heap.length / 2) {
      ...
      if (heap[child].priority < heap[parent].priority) {
        swapRemove(parent, child); // Swap item, key, and tag of heap[i] and heap[child]
        parent = child;
      } else {
        break;
      }
  ...
  private synchronized void swapRemove(int i, int j) {
    HeapNode<T> node = heap[i];
    heap[i] = heap[j];
    heap[j] = node;
  }
  
  private void swapAdd(int i, int j) {
    HeapNode<T> node = heap[i];
    heap[i] = heap[j];
    heap[j] = node;
  }
