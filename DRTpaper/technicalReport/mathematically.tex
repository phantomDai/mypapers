...
\begin{enumerate}[itemindent=1em]
  ...
  \item \textbf{当~$p_i > p_M$~时}，由于~$2(p_i(n)\theta_i - p_M(n)\theta_M)(m + \varepsilon) < 2(p_i(n) - p_M(n))\theta_i(m + \varepsilon)$~，只要保证~$p_i(n) - p_M(n) > 2(p_i(n) - p_M(n))\theta_i(m + \varepsilon)$~ 成立，则(6) 式成立，可得：
    \begin{equation}
      \displaystyle\frac{m + \varepsilon}{\varepsilon} < \displaystyle\frac{1}{2\theta_i}
    \end{equation}
  \item \textbf{当~$p_i < p_M$~时}，公式(6)可以转化为~$(p_M(n) - p_i(n))\varepsilon < 2(p_M(n)\theta_M - p_i(n)\theta_i)(m + \varepsilon)$~，$\because 2(p_M(n)\theta_M - p_i(n)\theta_i)(m + \varepsilon) > 2(p_M(n) - p_i(n))\theta_M(m + \varepsilon)$~，只要保证~$2(p_M(n) - p_i(n))\theta_M(m + \varepsilon) >(p_M(n) - p_i(n))\varepsilon$~则(6)式成立，可得：
    \begin{equation}
      \displaystyle\frac{m + \varepsilon}{\varepsilon} > \displaystyle\frac{1}{2\theta_M}
    \end{equation}
\end{enumerate}

综上所述，只要
\begin{equation}
  \displaystyle\frac{1}{2\theta_M} < \displaystyle\frac{m + \varepsilon}{\varepsilon} < \displaystyle\frac{1}{2\theta_{\Delta}}
\end{equation}
