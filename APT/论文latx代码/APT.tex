...
\markboth{IEEE Transactions on Reliability,~submitted}%
...
%method
We particularly develop two concrete algorithms,
...
%conclusion
It is demonstrated that the proposed adaptive partition testing is an effective testing approach, taking advantages of both random testing and partition testing. Our study also points out some interesting research directions for further improving the testing effectiveness of related techniques.
\end{abstract}
...
\textit{Acronyms\footnote{The singular and plural of an acronym are always spelled the same.}}

\begin{tabular}{ll}
APT		& Adaptive Partition Testing \\
MAPT	& Markov-chain Adaptive Partition Testing \\
RAPT	& Reward-punishment Adaptive Partition Testing \\
...
$m$			& Number of partitions of an input domain \\
$c_i$		& The $i_{\mathrm{th}}$ partition of an input domain \\
$p_i$		& The probability of $c_i$ to be selected for conducting a test \\
$p'_i$		& The updated probability of $c_i$ to be selected for conducting a test \\
$p_{i,j}$	& The transition probability, that is, the probability of transitioning from one state $i$ to another state $j$ \\
$p'_{i,j}$	& The updated transition probability, that is, the probability of transitioning from one state $i$ to another state $j$ \\
$P$			& Markov matrix \\
$Rew_i$		& Reward factor for $c_i$ in RAPT \\
$Pun_i$		& Punishment factor for $c_i$ in RAPT \\
$Bou_i$		& Bound value for $c_i$ in RAPT \\
\end{tabular}
...
\begin{figure}[tp]
	\centering
	\includegraphics[width=0.5\textwidth]{grepF}
	(a) \texttt{grep}
	\includegraphics[width=0.5\textwidth]{gzipF}
	(b) \texttt{gzip}
	\includegraphics[width=0.5\textwidth]{makeF}
	(c) \texttt{make}
...
\begin{figure}[tp]
	\centering
	\includegraphics[width=0.5\textwidth]{grepF2}
	(a) \texttt{grep}
	\includegraphics[width=0.5\textwidth]{gzipF2}
	(b) \texttt{gzip}
	\includegraphics[width=0.5\textwidth]{makeF2}
	(c) \texttt{make}
...
\begin{figure}[tp]
	\centering
	\includegraphics[width=0.5\textwidth]{grepT}
	(a) \texttt{grep}
	\includegraphics[width=0.5\textwidth]{gzipT}
	(b) \texttt{gzip}
	\includegraphics[width=0.5\textwidth]{makeT}
	(c) \texttt{make}
...
\begin{figure}[tp]
	\centering
	\includegraphics[width=0.5\textwidth]{grepT2}
	(a) \texttt{grep}
	\includegraphics[width=0.5\textwidth]{gzipT2}
	(b) \texttt{gzip}
	\includegraphics[width=0.5\textwidth]{makeT2}
	(c) \texttt{make}
