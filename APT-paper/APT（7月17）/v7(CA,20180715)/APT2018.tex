...
\usepackage{booktabs}%added for section 2.4
...
while the latter is based on a reward and punishment mechanism.
%results
...
\end{cases},
\end{equation}
\textcolor{blue}{where $\gamma$ denotes a adjusting parameter,} and then we set
...
\end{cases},
\end{equation}
\textcolor{blue}{where $\tau$ denotes a adjusting parameter,} and then we have
...
\textcolor{blue}{In this study, we consider two testing scenarios: (\romannumeral1) when a fault is detected, the testing process is paused and debugging is started, and (\romannumeral2) when a fault is detected, the testing process continues without a pause.}
...
%Table~\ref{tab:PstaF} to~\ref{tab:PstaT2}.
